...
public class SpawnExplosionScript : MonoBehaviour {
	...
	public void SpawnVFX () {
		...
		if (cameraShake && cameras != null) {
			var camShakeScript = cameras.GetComponent<CameraShakeScript> ();
			var simpleCamShakeScript = cameras.GetComponent<CameraShakeSimpleScript> ();
			if(camShakeScript!= null)
				camShakeScript.ShakeCamera (shakeAmount, shakeDuration);
			if (simpleCamShakeScript != null)
				simpleCamShakeScript.ShakeCamera ();
		}
